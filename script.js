...
async function muatData() {
  ...
  document.getElementById("db-status-dot").className =
    "w-1.5 h-1.5 rounded-full bg-green-500 shadow-[0_0_8px_green]";
  document.getElementById("db-status-text").innerText = "Hub Active";
  renderAll();
}
    ...
    area.innerHTML = `
            <div class="dynamic-island p-7 flex justify-between items-center fade-in border border-white/10">
                <div class="truncate mr-4">
                    ...
                    <h2 class="text-lg font-black tracking-tight truncate uppercase">${upcoming.content}</h2>
                </div>
        ...
        .map((t) => {
          ...
          return `<div class="glass-card p-4 flex justify-between items-center ${t.is_done ? "opacity-30" : ""}" style="border-left: 4px solid ${t.priority === "High" ? "#ff3b30" : t.priority === "Medium" ? "#ff9500" : "#8e8e93"}">
            <div class="flex items-center gap-4 truncate flex-1">
                ...
                <div class="truncate"><p class="text-[7px] font-black opacity-30 uppercase">${t.category} • ${t.tgl_deadline}</p><p class="font-bold truncate text-xs">${t.content}</p></div>
            </div>
...
function showCalendarDetail(dateStr, tasks) {
  ...
  document.getElementById("calendar-task-list").innerHTML = tasks
    .map((t) => {
      const hasLink = t.task_link && t.task_link.startsWith("http");
      return `<div class="p-4 bg-white/5 rounded-2xl border border-white/5">
...
async function simpanData() {
  ...
  await sb
    ...
    .insert([
      {
        content: isi,
        tgl_start: document.getElementById("tglMulai").value || t2,
        tgl_deadline: t2,
...
function renderPicker() {
  const cont = document.getElementById("datepicker-days"),
    ...
    m = pDate.getMonth(),
    y = pDate.getFullYear();
  lbl.innerText = pDate.toLocaleString("en-US", {
    month: "short",
    year: "numeric",
  });
  const first = new Date(y, m, 1).getDay(),
  ...
  for (let d = 1; d <= days; d++) {
    ...
    cont.innerHTML += `<div class="p-2 hover:bg-white/10 rounded-full cursor-pointer" onclick="selectDate('${dStr}')">${d}</div>`;
  }
}
...
function openSelect(type) {
  ...
  items.forEach((item) => {
    ...
    b.onclick = () => {
      if (type === "kategori")
        document.getElementById("btn-kategori").innerText = selectedKat = item;
      else
        document.getElementById("btn-priority").innerText = selectedPri = item;
      closeModal();
    };
    opt.appendChild(b);
  });
