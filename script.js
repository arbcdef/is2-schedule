...
// DATE PICKER LOGIC (FIXED FOR MOBILE)
...
// Menutup picker jika klik di luar area
document.addEventListener('click', (e) => {
    const p = document.getElementById("custom-datepicker");
    const input = document.getElementById("tglDeadline");
    if (!p.contains(e.target) && e.target !== input) {
        closePicker();
    }
});
...
function renderPicker() {
    ...
    for (let d = 1; d <= days; d++) {
        ...
        cont.innerHTML += `<div class="picker-day ${dStr === selFullDate ? "picker-selected" : ""}" 
...
function openSelect(type) {
    ...
    list.forEach((item) => {
        ...
        b.className = "option-btn py-4 bg-zinc-500/10 rounded-2xl font-bold text-sm hover:bg-blue-500 hover:text-white transition-all";
        b.innerText = item;
        b.onclick = () => {
            if (type === "kategori") {
                selectedKat = item;
    ...
    document.getElementById("listData").innerHTML = allTasks.map(t => `
        <div class="ios-card flex justify-between items-center mb-4 ${t.is_done ? "opacity-30" : ""}" 
            ...
            <div class="flex items-center gap-4">
                <input type="checkbox" ${t.is_done ? "checked" : ""} onclick="toggleDone(${t.id}, ${t.is_done})" class="w-6 h-6 rounded-full border-2">
                <div>
            ...
            <button onclick="askDel(${t.id})" class="bg-red-500/10 text-red-500 px-4 py-2 rounded-xl text-[10px] font-black">DELETE</button>
        </div>`).join("");
}
